...
static void write_bmp_header(struct bmp_header *bmp_h, FILE *fp)
{
...
static void write_dib_header(struct dib_header *dib_h, FILE *fp)
{
...
static struct pixel color_to_pixel(struct color c)
{
    return (struct pixel){.r = c.r, .g = c.g, .b = c.b};
}

static void write_pixel(struct pixel p, FILE *fp)
{
    fputc(p.b, fp);
    fputc(p.g, fp);
    fputc(p.r, fp);
...
static void write_row_padding(int padding_size, FILE *fp)
{
...
static void write_bitmap(Bitmap_t bitmap, FILE *fp)
{
    ...
    for (int row = 0; row < h; row++)
    {
        ...
        {
            struct pixel p = color_to_pixel(bitmap_get_color_at(bitmap, row, col));
            write_pixel(p, fp);
        }
        write_row_padding(padding_size, fp);
    }
}
...
void save_to_bmp(Bitmap_t bitmap, const char *file_name)
{
    ...
    int pixel_array_size = row_size * bitmap_height(bitmap);
    ...
    dib_h.width = bitmap_width(bitmap);
    dib_h.height = bitmap_height(bitmap);
